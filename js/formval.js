...
function validatePasswords(valfield1,   // first element to be validated
                           valfield2,  // second element to be validated
                           infofield1,  // first id of element to receive info/error msg
                           infofield2 ) // second id of element to receive info/error msg
{
  // check to make sure both are equal
  if (valfield1.value != valfield2.value) {
    msg (infofield1, "error", "Passwords do not match");
    msg (infofield2, "error", "Passwords do not match");
    return false;
  }
  // check length
  if (valfield1.value.length < 5) {
    msg (infofield1, "error", "Password length < 5");
    return false;
  }
  return true;
}
...
function validateUser    (valfield,   // element to be validated
                         infofield,  // id of element to receive info/error msg
                         required)   // true if required
{
alert(valinfo.value);
	...
	ajaxRequest.onreadystatechange = function() {
		if(ajaxRequest.readyState == 4) {
			if (ajaxRequest.responseText == "1") {
				msg (infofield, "error", "Username Unavailable");
				return false;
			} else {
				return true;
			}
	...
	var queryString = "?user=" + valfield.value;
	ajaxRequest.open("GET", "chk_user.php" + queryString, true);
