...
function db_connection() {
	...
	//make connection
	if (!($database_connection = @ocilogon(DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME))) {
...
function db_query($query, $dieonerror=1) {
	...
	$stmt = @ociparse($database_connection, $query);
	ociexecute($stmt);
...
function db_simplequery($table, $field, $haystack, $needle) {
	...
	$sql = "SELECT $field FROM $table WHERE $haystack = $needle LIMIT 1;";

	$stmt = @oci_parse($database_connection, $sql);
	oci_execute($stmt);
